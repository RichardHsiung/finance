...
DELETE_DATA_URL = 'https://gw.wmcloud.com/mercury/api/databooks'
...
def delete_file(cookies, filename):
    url = DELETE_DATA_URL
    dataurl = url + '/' + filename
    response = requests.delete(dataurl, cookies=cookies)
    if not response.ok:
        print("Can not delete the file: " + filename)
    else:
        print("Delete the file: " + filename)
